...
async function main() {
  ...
  try {
    ...
    const rows = Object.entries(map)
      .map(([t, v]) => ({ t, n: "", price: Number(v.price), ytd: Number(v.chYTD), chg: Number.isFinite(Number(v.change)) ? Number(v.change) : null }))
      .filter(r => Number.isFinite(r.price) && Number.isFinite(r.ytd) && r.price >= 1 && r.ytd > 100 && r.ytd <= YTD_SANITY_CAP)
      .sort((a, b) => b.ytd - a.ytd)
      .slice(0, 50)
      .map(r => ({ ...r, price: +r.price.toFixed(2), ytd: +r.ytd.toFixed(2), chg: r.chg != null ? +r.chg.toFixed(2) : null }));
    ...
    starGainers = Object.entries(newStars)
      .map(([t, stars]) => ({ t, stars, since: newSince[t] || null, price: +Number(map[t].price).toFixed(2),
        ...
        chg: Number.isFinite(Number(map[t].change)) ? +Number(map[t].change).toFixed(2) : null }))
    ...
    allGreen = Object.entries(map)
      .map(([t, v]) => ({ t, price: P(v.price), w: P(v.ch1w), m1: P(v.ch1m), m3: P(v.ch3m), m6: P(v.ch6m), ytd: P(v.chYTD), y1: P(v.ch1y), chg: P(v.change) }))
      .filter(r => r.price >= 1 && r.ytd != null && r.ytd <= YTD_SANITY_CAP
        && [r.w, r.m1, r.m3, r.m6, r.ytd, r.y1].every(x => x != null && x > 0))
      .sort((a, b) => b.y1 - a.y1)
      .slice(0, 50)
      .map(r => ({ ...r, price: +r.price.toFixed(2), w: +r.w.toFixed(1), m1: +r.m1.toFixed(1), m3: +r.m3.toFixed(1), m6: +r.m6.toFixed(1), ytd: +r.ytd.toFixed(1), y1: +r.y1.toFixed(1), chg: r.chg != null ? +r.chg.toFixed(2) : null }));
